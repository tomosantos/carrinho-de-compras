...
function preencherCarrinho(produtos) {
  ...
  produtos.forEach((data) => {
    addCarrinho(data.title, data.image, data.description, data.price)
  })

  atualizarCarrinho()
}

function addCarrinho(nome, imgLink, descricao, preco) {
  carrinho.push({
    'nome': nome,
    'imgLink': imgLink,
    'descricao': descricao,
...
function atualizarCarrinho() {
  ...
  items.textContent = `${carrinho.length} `

  carrinho.forEach((produto, index) => {
    carrinhoHTML.innerHTML += `<div
      id='produto${index}'
